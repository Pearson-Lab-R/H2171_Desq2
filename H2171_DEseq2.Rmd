...
genes_of_interest <- c("ASCL1", "NEUROD4", "YAP1","ZEB1","ITGB4")
heatmap_data <- cleaned_vsd[cleaned_vsd$symbol %in% genes_of_interest, ]

rownames(heatmap_data)<- heatmap_data$symbol
expr_cols <- setdiff(colnames(heatmap_data), c("symbol", "ensembl_id", "gene_id"))
heatmap_expr <- heatmap_data[, expr_cols]
